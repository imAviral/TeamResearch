...
# reading file data
soybean_oil_data <- read_csv("../individual_data/Soybean_Oil_data.csv",show_col_types = FALSE)
# Check the structure of the data
str(soybean_oil_data)

# Ensure the date column is in Date format
soybean_oil_data$date <- as.Date(soybean_oil_data$date, format = "%Y-%m-%d")

# Aggregate the data to get the average close for each date (if there are duplicates)
soybean_oil_data_agg <- soybean_oil_data %>%
  group_by(date) %>%
  summarise(close = mean(close, na.rm = TRUE))

# Plot close vs date as a line graph
ggplot(soybean_oil_data_agg, aes(x = date, y = close)) +
  geom_line(color = "#F1C232") +  # Line graph
  labs(title = "Close vs Date", x = "Date", y = "Close") +
  theme_minimal()

# Plot histogram with density curve line
ggplot(soybean_oil_data_agg, aes(x = close)) +
